...
public class PositionMgService extends OrgBaseService implements IPositionMgService {
	...
	public void clonePermissions(Long positionid, String[] poids, IUser opUser) {
		...
		List<PositionAuthrity> pas = hibernateDao
				.createQuery(
						"from PositionAuthrity pa where pa.id.taposition.positionid=? and pa.usepermission=? and (pa.effecttime is null or pa.effecttime >=?)",
						new Object[] { positionid, "1", timeService.getSysTimestamp() }).list();
	...
	public List<UserInfoVO> queryUsers(ParamDTO dto) {
		...
		List<UserInfoVO> users = hibernateDao.createQuery(sb.toString(), new Object[] { "0", "2", timeService.getSysDate(), "0", "1", orgid }).list();
		return users;
	}
	...
	public PageBean queryUsersByDto(ParamDTO dto) {
		...
		String countHql = finder.getRowCountHql();
		Query queryCount = hibernateDao.createQuery(countHql, new Object[0]);
		query = hibernateDao.createQuery(sb.toString(), new Object[0]);
		if (ValidateUtil.isNotEmpty(username)) {
			query.setString("username", "%" + username + "%");
			queryCount.setString("username", "%" + username + "%");
		}
			...
			if (!list.contains(child)) {
				...
				if ("2".equals(positionType)) {
					...
					User user = (User) hibernateDao.createQuery(
									...
									new Object[] { child.getPositionid(), "2" }).uniqueResult();
		...
		if (IPosition.ADMIN_POSITIONID.equals(positionid)) {
			return hibernateDao.createQuery(
					"from " + getEntityClassName(Position.class) + " p where p.effective=? and p.taorg.orgidpath like (select o.orgidpath from "
							+ getEntityClassName(Org.class) + " o where o.orgid=?)||'%'", new Object[] { "0", orgid }).list();
		}
		return hibernateDao
				.createQuery(
						"from "
								...
								+ getEntityClassName(Org.class) + " o where o.orgid=?)||'%'", new Object[] { "0", positionid, orgid }).list();
	}
	...
	public void saveSharePositions(ParamDTO dto, List<Key> orgids) {
		...
		List<Menu> menus = hibernateDao.createQuery(
				...
				new Object[] { sharePosition.getPositionid() }).list();
		...
		for (Key key : orgids) {
			...
			if ("true".equals(check)) {
				...
				List<Position> pList = hibernateDao.createQuery(
						"select p from SharePosition sp," + super.getEntityClassName(Position.class) + " p," + super.getEntityClassName(Org.class)
								+ " o where sp.id.spositionid=? and sp.id.dpositionid=p.positionid and p.taorg.orgid=? and p.iscopy=?",
						new Object[] { sposition, orgid, "1" }).list();
				if (!ValidateUtil.isEmpty(pList)) {
					Position dposition = (Position) pList.get(0);
					if (dposition.getOrgid().equals(org.getOrgid())) {
						hibernateDao.createQuery("delete from SharePosition sp where sp.id.spositionid=? and sp.id.dpositionid=?",
								new Object[] { sposition, dposition.getPositionid() }).executeUpdate();

						List<Menu> list = hibernateDao.createQuery(
								"select distinct m from " + super.getEntityClassName(Menu.class)
										+ " m,PositionAuthrity pa where pa.id.taposition.positionid=? and m.menuid=pa.id.tamenu.menuid",
								new Object[] { dposition.getPositionid() }).list();
						for (Menu m : list) {
							grantService.permissionChangeUniteFunction(new PermissionInfoVO(m.getMenuid(), dposition.getPositionid(), dto
									.getUserInfo().getUserid(), curDate, "0", "6"));

							hibernateDao
									.createQuery(
											"delete from DataAccessDimension dad where dad.positionid=? and dad.menuid=? and dad.dimensiontype=? and dad.syspath=?",
											new Object[] { dposition.getPositionid(), m.getMenuid(), "YAB139", curSyspathId }).executeUpdate();
						...
						hibernateDao.createQuery("delete from UserPosition up where up.id.taposition.positionid=?",
								new Object[] { dposition.getPositionid() }).executeUpdate();
	...
	public List<Org> queryCopyPositionInOrgBySharePositionId(Long positionid) {
		...
		return hibernateDao.createQuery(
				"select distinct o from SharePosition sp," + getEntityClassName(Position.class) + " p," + getEntityClassName(Org.class)
						+ " o where sp.id.spositionid=? and sp.id.dpositionid=p.positionid and p.taorg.orgid=o.orgid", new Object[] { positionid })
				.list();
	}
	...
	public void saveEffectiveTimePanel(ParamDTO dto) {
		...
		PositionAuthrity pa = (PositionAuthrity) hibernateDao.createQuery(
				"from PositionAuthrity pa where pa.id.taposition.positionid=? and pa.id.tamenu.menuid=?", new Object[] { positionid, menuid })
				.uniqueResult();
	...
	public Date queryEffectiveTime(Long menuid, Long positionid) {
		return (Date) hibernateDao.createQuery(
				"select pa.effecttime from PositionAuthrity pa where pa.id.tamenu.menuid=? and pa.id.taposition.positionid=?",
				new Object[] { menuid, positionid }).uniqueResult();
	}
