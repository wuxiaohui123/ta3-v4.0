...
public class UserMgServiceImpl extends OrgBaseService implements IUserMgService {
	...
	public void deleteUsers(List<Key> users, ParamDTO dto) {
		...
		if (!ValidateUtil.isEmpty(users)) {
			...
			for (Key key : users) {
				...
				hibernateDao.createQuery("delete from UserPosition up where up.id.tauser.userid=?", new Object[] { userid }).executeUpdate();
				for (IPosition position : ps) {
					List<PositionAuthrity> pas = hibernateDao.createQuery("from PositionAuthrity pa where pa.id.taposition.positionid=?",
							new Object[] { position.getPositionid() }).list();
					...
					hibernateDao.createQuery("delete from AdminYab003Scope ays where ays.id.positionid=?", new Object[] { position.getPositionid() })
							.executeUpdate();

					hibernateDao.createQuery("update " + getEntityClassName(Org.class) + " o set o.orgmanager=null where o.orgmanager=?",
							new Object[] { position.getPositionid() }).executeUpdate();

					hibernateDao.createQuery("delete from ManagerMg mm where mm.id.positionid=?", new Object[] { position.getPositionid() })
							.executeUpdate();

					hibernateDao.createQuery("delete from OrgMg om where om.id.positionid=?", new Object[] { position.getPositionid() })
							.executeUpdate();
	...
	public void updateUser(ParamDTO dto) {
		...
		if (!oldUserName.equals(newUserName)) {
			List<Position> ps = hibernateDao.createQuery(
					"select distinct p from " + getEntityClassName(Position.class)
							+ " p,UserPosition up where p.positiontype=? and p.positionid=up.id.taposition.positionid and up.id.tauser.userid=?",
					new Object[] { "2", dto.getAsLong("userid") }).list();
		...
		if (!ValidateUtil.isEmpty(list)) {
			for (Object o : list) {
				...
				for (int i = 0; i < pField.length; i++) {
					...
					try {
						PropertyDescriptor pd = new PropertyDescriptor(pField[i].getName(), o.getClass());
						Object invoke = pd.getReadMethod().invoke(o, new Object[0]);
						if (invoke != null) {
							if ("directorgid".equals(fieldName)) {
								String orgnamepath = (String) hibernateDao.createQuery(
										"select o.orgnamepath from " + getEntityClassName(Org.class) + " o where o.orgid=?", new Object[] { invoke })
										.uniqueResult();
				...
				if (!ValidateUtil.isEmpty(sField)) {
					for (int i = 0; i < sField.length; i++) {
						String fieldName = sField[i].getName();
						try {
							PropertyDescriptor pd = new PropertyDescriptor(sField[i].getName(), o.getClass());
							Object invoke = pd.getReadMethod().invoke(o, new Object[0]);
							if (invoke != null) {
								map.put(fieldName, invoke.toString());
							}
	...
	public boolean checkSameUserId(ParamDTO dto) {
		...
		Query users = hibernateDao
				.createQuery("from " + getEntityClassName(User.class) + " u where u.loginid=:loginid and (u.destory is null or u.destory=:destory)",
						new Object[0]).setParameter("loginid", loginId).setString("destory", "1");
	...
	public boolean checkOrgInUserCurrentPositionOrg(ParamDTO dto) {
		...
		List<Object[]> list = hibernateDao.createQuery(hql.toString(), new Object[] { orgidNow, dto.getAsLong("orgid"), "0" }).list();
		if (list.size() > 0)
			return true;
		return false;
	}
	...
	public List<Position> getCurrentUserCanDistributionUserPersitions(ParamDTO dto) {
		...
		List<UserPosition> ups = hibernateDao.createQuery("from UserPosition up where up.id.tauser.userid = ?",
				new Object[] { dto.getAsLong("userid") }).list();
		...
		PositionInfoVO pi;
		for (Iterator i$ = cpp.iterator(); i$.hasNext();) {
			pi = (PositionInfoVO) i$.next();
			for (UserPosition up : ups) {
				if (pi.getPositionid() == up.getId().getTaposition().getPositionid())
	...
	public List<PositionInfoVO> getCurrentUserCanDistributionUserPositionsByOrg(ParamDTO dto) {
		...
		PositionInfoVO pi;
		for (Iterator i$ = cupbo.iterator(); i$.hasNext();) {
			pi = (PositionInfoVO) i$.next();
			for (Position up : cup) {
				if (pi.getPositionid() == up.getPositionid())
					re.add(pi);
			}
		}
	...
	public List<UserPerrmissionVO> queryUserPerrmission(Long userid) {
		...
		sbf.append(
				"select distinct new com.yinhai.ta3.system.org.domain.UserPerrmissionVO(m.menuname,m.url, m.resourcetype, pa.usepermission, pa.repermission, pa.reauthrity, p.positionname, u1.name, pa.createtime, p.orgnamepath,pa.auditstate,m.menuid)")
		...
		return hibernateDao.createQuery(sbf.toString(), new Object[] { userid, "0", timeService.getSysDate(), "0" }).list();
	}
	...
	public void resetPassword(ParamDTO dto) {
		...
		for (int i = 0; i < users.size(); i++) {
			...
			if ((userid != null) && (ValidateUtil.isNotEmpty(dto.getAsString("newPassword")))) {
	...
	public void updateDirectAndAffiliatedOrgs(Long orgid, List<Key> ids, Long userid, IUser userInfo) {
		...
		if (!orgid.equals(u.getDirectorgid())) {
			...
			List<UserPosition> ups = hibernateDao.createQuery(
					...
					new Object[] { userid, u.getDirectorgid(), "2" }).list();
			if (ups != null) {
				...
				List upList = hibernateDao.createQuery("from UserPosition up where up.id.tauser.userid=? and up.mainposition=?",
		...
		if (!ValidateUtil.isEmpty(ids)) {
			for (Key key : ids) {
				...
				if ("true".equals(key.getAsString("checked"))) {
					...
					List<UserPosition> ups = hibernateDao
							.createQuery(
									"select up from UserPosition up,"
											+ getEntityClassName(Position.class)
											+ " p where up.id.taposition.positionid=p.positionid and up.id.tauser.userid=? and p.taorg.orgid=? and p.positiontype=?",
									new Object[] { userid, key.getAsLong("id"), "2" }).list();
					if (ups != null) {
						Long positionid = ((UserPosition) ups.get(0)).getId().getTaposition().getPositionid();
						hibernateDao.createQuery("delete from UserPosition up where up.id.taposition.positionid=? and up.id.tauser.userid=?",
								new Object[] { positionid, ((UserPosition) ups.get(0)).getId().getTauser().getUserid() }).executeUpdate();
			...
			if ((upList == null) || (upList.size() <= 0)) {
				...
				hibernateDao.createQuery(
						"update UserPosition up set up.mainposition=? where up.id.tauser.userid=? and up.id.taposition.positionid=?",
						new Object[] { "1", userid, position.getPositionid() }).executeUpdate();
			}
		}
	...
	public PageBean queryUserOpLogs(String gridId, ParamDTO dto) {
		...
		Query query = hibernateDao.createQuery(sb.toString(), new Object[0]).setFirstResult(skipResults.intValue())
