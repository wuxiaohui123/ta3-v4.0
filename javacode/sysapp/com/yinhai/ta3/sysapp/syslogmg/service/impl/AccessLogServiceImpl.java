...
public class AccessLogServiceImpl extends WsBaseService implements IAccessLogService {
	...
	public PageBean queryAccessInfo(Date startDate, Date endDate, Integer start, Integer limit, Long curUser, Long positionid) {
		...
		hql1.append("select al.logid from AccessLog al,").append(super.getEntityClassName(User.class.getName())).append(" u,")
				.append(super.getEntityClassName(Position.class.getName())).append(" p,").append(super.getEntityClassName(Menu.class.getName()))
				.append(" m");

		if (!IUser.ROOT_USERID.equals(curUser)) {
			hql1.append(" where 1=1 and p.taorg.orgid in(").append("select om.id.orgid from ").append(super.getEntityClassName(Org.class.getName()))
					.append(" o,OrgMg om where om.id.positionid=" + positionid + ")");
		} else {
			hql1.append(" where 1=1");
		}
		hql1.append(" and al.ispermission=" + ispermission + " and al.userid=u.userid and al.positionid=p.positionid and al.permissionid=m.menuid ");
		hql.append(
				...
				.append(super.getEntityClassName(User.class.getName())).append(" u,").append(super.getEntityClassName(Position.class.getName()))
				.append(" p,").append(super.getEntityClassName(Menu.class.getName())).append(" m");

		if (!IUser.ROOT_USERID.equals(curUser)) {
			hql.append(" where 1=1 and p.taorg.orgid in(").append("select om.id.orgid from ").append(super.getEntityClassName(Org.class.getName()))
					.append(" o,OrgMg om where om.id.positionid=" + positionid + ")");
		} else {
			hql.append(" where 1=1");
		}
		hql.append(" and al.ispermission=" + ispermission + " and al.userid=u.userid and al.positionid=p.positionid and al.permissionid=m.menuid ");
		...
		query = hibernateDao.createQuery(hql.toString(), new Object[0]).setFirstResult(skipResults.intValue()).setMaxResults(maxResults.intValue());
		Finder finder = Finder.create(hql1.toString());
		String countHql = finder.getRowCountHql();
		queryCount = hibernateDao.createQuery(countHql, new Object[0]);
		Long total = (Long) queryCount.uniqueResult();
