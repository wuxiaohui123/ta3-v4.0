...
public class WebUtil {
	...
	public static IUserAccountInfo getUserAccountInfo(HttpServletRequest request) {
		return (IUserAccountInfo) request.getSession().getAttribute("ta3.userinfo");
	}

	public static IMenu getCurrentMenu(HttpServletRequest request) {
		return (IMenu) request.getSession().getAttribute("ta3.currentmenu");
	}

	public static List<IMenu> getCurrentUserPermissionMenus(HttpSession session) {
		return (List<IMenu>) session.getAttribute("__USER_PERVIEW_MENUS_FLAG__");
	}

	public static Set<String> getCurrentUserPermissionUrls(HttpSession session) {
		return (Set<String>) session.getAttribute("__USER_PERVIEW_FLAG__");
	}

	public static boolean isAdministrator(HttpServletRequest request) {
		IPermissionServcie ps = (IPermissionServcie) ServiceLocator.getService("permissionServcie");
	...
	public static List<IPosition> getUserPositions(HttpServletRequest request) {
		...
		List<IPosition> list = (List<IPosition>) request.getAttribute(IConstants.USER_EFFECTIVE_POSITIONS);
		for (int i = 0; i < list.size(); i++) {
			IPosition iposition = ConvertUtil.ObjectToPosition(list.get(i));
			if (iposition != null) {
	...
	public static List<IPosition> getCurrentUserPositions(HttpSession session) {
		return (List<IPosition>) session.getAttribute("__USER_EFFECTIVE_POSITIONS__");
	}
