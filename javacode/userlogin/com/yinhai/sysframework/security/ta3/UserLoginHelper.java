...
public class UserLoginHelper implements IUserLogin {
	...
	private static Logger log = LogManager.getLogger(UserLoginHelper.class.getName());
	...
	IPermissionServcie permissionServcie;
	...
	public void regesitUserAccount(IUserAccountInfo userAccount, HttpServletRequest request) {
		...
		session.setAttribute("ta3.userinfo", userAccount);
		session.setAttribute("__USER_PERVIEW_FLAG__", loadPermissionUrls(userAccount.getUser()));
		session.setAttribute("__USER_PERVIEW_MENUS_FLAG__", loadPermissionMenus(userAccount.getUser()));
		session.setAttribute("__USER_EFFECTIVE_POSITIONS__", positions);
	...
	protected List<IMenu> loadPermissionMenus(IUser user) {
		return permissionServcie.getUserPermissionMenus(user.getUserid());
	}

	protected Set<String> loadPermissionUrls(IUser user) {
		return permissionServcie.getUserPermissionUrl(user.getUserid());
	}

	protected List<IPosition> loadUserEffectivePositions(Long userid) {
		return permissionServcie.getUserEffectivePositions(userid);
	}
	...
	protected String getRequestAddress(HttpServletRequest request){
		...
		if ((str != null) && (str.endsWith("logon.do"))) {
	...
	protected String getServerAndHost(HttpServletRequest request){
	    ...
	    try{
	      ...
	      if (null != localUnknownHostException) {
	...
	public IPermissionServcie getPermissionServcie() {
		return permissionServcie;
	}
