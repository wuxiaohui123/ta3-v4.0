...
<body class="no-scrollbar">
	...
	<ta:form id="form1" fit="true">
		...
		<ta:panel id="panel1" key="基本信息" cols="3">
			...
			<ta:text id="aae003" key="最大缴费年月" readOnly="true"  labelWidth="120" cssInput="color:red"/>
			<ta:text id="aae240" key="账户余额" readOnly="true"  labelWidth="120" cssInput="color:red"/>
		</ta:panel>
		<ta:panel id="panel2" key="就诊信息" cols="3">
			<ta:selectInput id="aka130" key="医疗类别" collection="AKA130" required="true" labelWidth="120" filter="51,52,71,72" maxVisibleRows="8" onSelect="fnSelectAka130"/> 
			...
			<ta:text id="akb020"  key="医疗机构编号"  required="true" labelWidth="120" />
			...
			<ta:text id="yke023" key="科别" labelWidth="120"/>
			<ta:selectInput id="aka120"  key="病种" labelWidth="120"  span="2"  maxVisibleRows="8" isAutoExtend="false" />
			<ta:text id="ake024" key="疾病诊断" labelWidth="120" span="3"/>
		</ta:panel>
		<ta:panel id="dyspxx" key="待遇审批信息" height="120px" cols="3">
			...
			<ta:text id="sfsqmxb" key="是否申请慢性病" readOnly="true" labelWidth="120" cssInput="color:red;"  cssStyle="color:red;"/>
...
<script type="text/javascript">
		...
		Base.getJson("<%=path%>/medicalbenefit/common/baseInfoAction!getPersonInfo.do",param,
			function(data){
				...
				if(data &&  data.personInfoDto){
					...
					Base.setValue("aae003",data.personInfoDto.aae003); // 最大缴费年月
					Base.setValue("aae240",data.personInfoDto.aae240); // 账户余额
					Base.setValue("sfsqmxb",data.personInfoDto.sfsqmxb); // 是否申请慢性病

					//获取aka130显示的值					
					Base.submit("aae140","centerCalRegAction!aka130list.do",null,null,false,null,null);
					// 检查是否在院
					
					Base.submit("aac001","centerCalRegAction!isInHospital.do",null,null,false,function(data1){
						// 焦点置于医疗类型
						Base.focus("aka130");
						// 同步获取相关数据 :1,人员待遇信息;2,申请备案信息
						getRelationData();
					},function(data2){
						Base.alert("当事人存在在院信息！","warn");
					});
	...
	**/
	function fnDate(o){
		...
		if(aae030 > aae031){
			...
			if("aae031" == id)   // 如果开始日期比结束日期大,且当前事件ID为开始日期,则清空结束日期,否则弹出提示框
	...
	**/
	function fnAkb020_y(key, value){
		...
		fn_getDysp();
		//如果是转院，则获取转院的诊断信息
	    var aka130 = Base.getValue("aka130"); //医疗类别
		// 区内转院和区外转院
		if("22" == aka130 || "24" == aka130 || "26" == aka130 || "26a" == aka130){
			var param = {};
			param["dto['aac001']"] = Base.getValue("aac001");
			param["dto['aka130']"] = Base.getValue("aka130");
			param["dto['akb020']"] = akb020;
			Base.submit("","centerCalRegAction!getKc20Detail.do",param);
		}
	}
	...
	**/
	function fnZY(){
		...
		var aka130 = Base.getValue("aka130");
		if("23" == aka130){
			Base.submit("aac001,aka130","centerCalRegAction!getKc20.do",null,null,false,function(){
				Base.hideObj("akb020");
				Base.showObj("akb020_y");
				Base.setEnable("akb020_y");
				Base.setValue("akb020_y",""); // 清空异地医院编码
				...
				Base.alert("没有获取到当前人员的转院备案信息！","warn");
				flag = false;
			});
		}
		return flag;
	...
	**/
	function fn_getDysp(){
		//先清空待遇信息
		Base.setValue("ykc239");
		Base.setValue("yka249");
		Base.setValue("yka119");
		Base.setValue("yka114");
		Base.setValue("yka115");
		Base.setValue("spqksm");
		//人员编号
		var aac001 = Base.getValue("aac001");
		//医疗类别 
		var aka130 = Base.getValue("aka130");
	...
	**/
	function toMxSh(){
		...
		Base.openWindow("win_mxlr", "明细录入信息", myPath() + "/medicalbenefit/calculation/centerCalRegAction!toMxSh.do", parameter, "99%", "99%", null, null, true, null, null);
	}
	...
	**/
	function toQueryJiaoFei(){
		...
		Base.openWindow("win_jf", "人员缴费信息", myPath() + "/medicalbenefit/calculation/centerCalRegAction!toQueryJiaoFei.do", parameter, "98%", "95%", null, null, true, "", ""); 
	}
</script>
