...
<%@ include file="/ta/inc.jsp"%>
	...
	<ta:pageloading />
		<ta:buttonLayout  id="tbr1"  >
			...
			<ta:toolbarItem id="btnZBJf"   key="批量整笔结算拒付"     icon="xui-icon-spotSave"  onClick="toJf('2')"/>
			<ta:toolbarItem id="btnQxJf"   key="批量取消拒付"   icon="xui-icon-spotDelete"  onClick="toQxJf()" />
			<ta:toolbarItem id="btnTg"     key="批量审核完成"       icon="xui-icon-spotSave"  onClick="toTg('2')" />
...
<!-- 			<ta:toolbarItem id="btnSet"   key="设置" icon="icon-setting"  onClick="toOpen()" /> -->
	...
	<ta:form id="form1" fit="true">
		<ta:panel id="fltCxtj" key="查询条件" cols="3" >
			...
			<ta:selectInput id="yka316"  key="清算类别"  labelWidth="120"  required="false" collection="YKA316"/>
			...
			<ta:selectInput id="yke090" key="审核状态"  labelWidth="120" collection="YKE090" />
		</ta:panel>
		<ta:panel id="pnlRYJS" key="人员结算信息(双击列表数据弹出对此笔结算的审核扣款页面)" fit="true" >
			<ta:datagrid id="dgRYJS" fit="true"  forceFitColumns="true" haveSn="true"  selectType="checkbox" onRowDBClick="openDeail"> 
			   ...
			   <ta:datagridItem id="yke090" key="审核标记"  align="center" dataAlign="left" maxChart="5"  collection="YKE090" formatter="fnFormatter" sortable="true"/>
			   <ta:datagridItem id="yke091" key="拒付标记"  align="center" dataAlign="left" maxChart="5"  collection="YKE091" formatter="fnFormatter" sortable="true"/>
			   <ta:datagridItem id="aka130" key="医疗类别"  align="center" dataAlign="left" maxChart="5"  collection="AKA130" formatter="fnFormatter" sortable="true"/>
			   ...
			   <ta:datagridItem id="akb020" key="医院编号"  hiddenColumn="false"/>
   ...
   function fn_getHosInfo(){
	...
	function query()
	{
		if(checkCxtj()){
			...
			Base.submit("fltCxtj","baseDetailAction!query.do",null,null,false,function(data){
				Base.setPanelTitle("pnlRYJS","人员结算信息,合计:"+data.fieldData.rows+"人次");
			});
		}
	}
	//拒付
	function openDeail(e,rowdata)
	{	
		var parameter={};
		parameter["dto['aaz217']"]=rowdata.aaz217; // 医疗待遇事件ID
		parameter["dto['aaz216']"]=rowdata.aaz216; // 医疗待遇事件ID
		parameter["dto['aaz208']"]=rowdata.aaz208; // 医疗待遇事件ID
		Base.openWindow("w_jf","拒付操作",myPath() + "/process/medicalbenefit/reckoning/baseDetailAction!toJf.do",parameter,"98%","99%",null,null,true,null,null);
	}
	//清空条件
	function  toClear()
	{
	...
	function toQxJf()
	{
		var data=Base.getGridSelectedRows("dgRYJS");
		if(1 > data.length){
			Base.alert("请选择数据！","warn");
			return false;
		}
		...
		var parameter={};
		parameter.gridData = Ta.util.obj2string(data);
		Base.submit("fltCxtj","baseDetailAction!toJfAll.do",parameter,null,false,null,null);
	}
	...
	function toTg(lx)
	{
		...
		var parameter={};
		parameter["dgrdJSLX"]=Ta.util.obj2string(data);
		parameter["dto.lx"]=lx;
		Base.submit("fltCxtj","baseDetailAction!toTgAll.do",parameter,null,false,null,null);
	}
	//批量整笔拒付
function toJf(lx)
{
	...
	Base.submit("fltCxtj","baseDetailAction!toSaveJfMore.do",parameter,null,false,null,null);
}	
	//取消审核
	function toQxSh()
	{
	...
	function toQueryDetail()
	{
		...
		if(data1.length > 0){
			...
			Base.openWindow('detailWin',"明细信息",myPath()+"/process/medicalbenefit/reckoning/baseDetailAction!checkDetail.do", params, "98%", "99%",null,function(){
			},true);
		}
	}
