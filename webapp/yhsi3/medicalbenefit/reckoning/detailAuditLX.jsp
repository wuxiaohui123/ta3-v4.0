...
<%@ include file="/ta/inc.jsp"%>
	...
	<ta:form id="form1" fit="true">
		...
		<ta:panel id="fltCxtj" key="查询条件" cols="3" >
		    <ta:text id="akb020" key="医疗机构编码" labelWidth="120"/>
			<ta:text id="akb021" key="医疗机构名称" readOnly="true" span="2" labelWidth="120"/>
			<ta:date id="aae030" key="开始日期"  showSelectPanel="true" labelWidth="120" />
			<ta:date id="aae031" key="终止日期"  showSelectPanel="true" labelWidth="120" />
			<ta:selectInput id="yka316" key="清算类别" collection="YKA316" labelWidth="120" filter="17,18,24" reverseFilter="true" maxVisibleRows="7"/>
		</ta:panel>
...
<script type="text/javascript">
	...
	function query()
	{
		...
		Base.submit("fltCxtj","detailAuditLXAction!toQueryHospitalInfo.do");
	}
	//表格颜色设置
	 ...
	 function fnClickDgrd1(e,data)
	{
		...
		//查询清算申请下的人员结算数据
		Base.submit("","detailAuditLXAction!checkCSXX.do",parameter,false,null,null);
	...
	function fnClickDgrd2(data,e)
	{
		...
		parent.Base.openWindow('detailWin',"明细信息","<%=path%>/process/medicalbenefit/reckoning/detailAuditLXAction!checkDetail.do", params, "95%", "90%",null,null,true);
	}
	//保存
	function toSave()
	{
		...
		if(1 > dgrdJSLX.length){
			...
			Base.confirm("确定要保存吗?",function(yes){
				if(yes){
					...
					Base.submit("form1","detailAuditLXAction!toSave.do",parameter,false,null,null);
				}
			}); 
		}
	}
