...
<%@ include file="/ta/inc.jsp"%>
	...
	<ta:form id="form1" fit="true">
		...
		<ta:panel id="pnlCxtj" key="查询条件" cols="7" >
			...
			<ta:selectInput id="yka316" key="人员类别" span="2" collection="YKA316" filter="13,14" reverseFilter="true"/>
		</ta:panel>
...
<script type="text/javascript">
	...
	function query()
	{
		...
		Base.submit('pnlCxtj','detailReviewMxbAction!toQueryHospitalInfo.do');
	}
	...
	function toSave()
	{
		...
		parent.Base.openWindow('w_sh',"医疗机构费用月度审核","<%=path%>/process/medicalbenefit/reckoning/detailReviewMxbAction!toOpenSh.do", parameter, "80%", "80%",null,null,true);
	}
	...
	function toCancel()
	{
		...
		if("" == dgrdJSLX || null == dgrdJSLX){
			...
			Base.submit("","detailReviewMxbAction!toCancel.do",parameter,null,false,function(){
	...
	function toPrint(data,e)
	{
		...
		if("" == dgrdJSLX || null == dgrdJSLX){
			...
			if("undefined" == typeof(aaz272) || "" == aaz272)
				return;
			$("#report1_printIFrame").attr("src","<%=path%>/reportServlet?action=2&name=report1&reportFileName=HosptialMonthAduit_mxb.raq&"+
	...
	function toSet(){
	   if(null == columnData || "" == columnData){
	    ...
	    openGridSet("dgrdJSLX",columnData,"menuid01031616",null,null,null);
	}
	var columnData = null;
</script>
