		...
		<ta:form id="form" fit="true" >
		   <ta:box id="bxID" cols="3" >
			...
			<ta:panel id="pnlZyxx" key="人员基本信息及就诊信息" cols="4">
				...
				<ta:text id="yka115" key="起付标准" readOnly="true" labelWidth="120"/>
				<ta:text id="aae030mztc" key="待遇开始时间"  readOnly="true" labelWidth="120" />
				<ta:text id="aae031mztc" key="待遇结束时间"  readOnly="true" labelWidth="120" />
				<ta:text id="akb021" key="就诊医院" readOnly="true" labelWidth="120" span="2"/>
				<ta:date id="aae030" key="就诊日期" readOnly="true" labelWidth="120"/>
				<ta:text id="akc193" key="诊断" readOnly="true" labelWidth="120" span="2"/>
...
function save()
{
	...
	// 保存数据
	Base.submit("","<%=path %>/process/medicalbenefit/reckoning/detailAuditMztcAction!toSh.do",parameter);
}
...
function toMxSh()
{
	...
	parent.Base.openWindow("w_mxsh","明细审核","<%=path %>/process/medicalbenefit/reckoning/detailAuditMztcAction!toOpenShMx.do", parameter, "99%", "99%",null,function(){
		Base.submit("aaz217","<%=path %>/process/medicalbenefit/reckoning/detailAuditMztcAction!getKc35ByAaz217.do");
	},true);	
}
