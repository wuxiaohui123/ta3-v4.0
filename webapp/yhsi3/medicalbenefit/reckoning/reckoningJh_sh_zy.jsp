		...
		<ta:form id="form" fit="true" >
		   <ta:box id="bxID" cols="3" >
			...
			<ta:panel id="pnlZyxx" key="个人住院信息" cols="4">
				...
				<ta:selectInput id="aka130" key="就诊类型" collection="AKA130" readOnly="true" labelWidth="120"/>
				...
				<ta:text id="aka121" key="出院病种" readOnly="true" labelWidth="120" span="2"/>
				...
				<ta:text id="ake053" key="统筹范围费用累计"  readOnly="true" labelWidth="120" />
				<ta:text id="yka436" key="统筹支付累计"  readOnly="true" labelWidth="120" />
			</ta:panel>
			...
			<ta:panel id="pnlSh" key="审核修改" expanded="false" cols="3" >
				...
				<ta:selectInput id="aka123" key="审核病种名称" labelWidth="120"  maxVisibleRows="8" isAutoExtend="false"  onSelect="fnSelect"/>
...
function save()
{
	...
	parameter["dto['aae013']"]=Base.getValue("aae013"); // 备注
	parameter["dto['aae017']"]=Base.getValue("aae017"); // 拒付说明
	parameter["dto['aac001']"]=Base.getValue("aac001"); // 人员ID
	// 保存数据
	Base.submit("","<%=path %>/process/medicalbenefit/reckoning/detailAuditZYAction!toSh.do",parameter);
}
...
function toMxSh()
{
	...
	parent.Base.openWindow("w_mxsh","明细审核","<%=path %>/process/medicalbenefit/reckoning/detailAuditZYAction!toOpenShMx.do", parameter, "99%", "99%",null,function(){
		Base.submit("aaz217","<%=path %>/process/medicalbenefit/reckoning/detailAuditZYAction!getKc35ByAaz217.do");
	},true);	
}
...
{
	Base.submit("aka123","<%=path %>/process/medicalbenefit/reckoning/detailAuditZYAction!getAka122ByAka120.do");
}
