		...
		<ta:form id="form" fit="true" >
		   <ta:box id="bxID" cols="3" >
			...
			<ta:box id="bxCxtj" cols="5">
			    ...
			    <ta:selectInput  id="ake003" key="项目类别"  collection="AKE003" labelWidth="80"/>
			    <ta:number  id="aka069" key="自付比例"  labelWidth="80" precision="2" max="1" min="0"/>
			    ...
			    <ta:checkboxgroup id="cbgps" cols="7" span="2" cssStyle="padding:0px;margin-top:5px;margin-left:16px;">
			       <ta:checkbox id="xj" key="限价"  labelStyle="padding:0px;font-size:15px;" />
...
function toJf(lx)
{
	...
	Base.submit("bxID","<%=path %>/process/medicalbenefit/reckoning/detailAuditCommAction!toSaveDetail.do",parameter,null,false,null,null);
}

// 审核
function toSh()
{
	Base.submit("bxID","<%=path %>/process/medicalbenefit/reckoning/detailAuditCommAction!toSaveShMx.do");
}
// 查询
function toQuery()
{
	...
	parameter["dto['ake003']"]=Base.getValue("ake003");
	...
	Base.submit("","<%=path %>/process/medicalbenefit/reckoning/detailAuditCommAction!toQueryShMx.do",parameter,null,false,null,null);
}
...
function fnDbClick(e,rowdata)
{
	...
	if(flag)
	   parameter["dto['jf']"]="1";
	Base.submit("","<%=path %>/process/medicalbenefit/reckoning/detailAuditCommAction!toQueryKc22.do",parameter,null,false,null,null);
}
