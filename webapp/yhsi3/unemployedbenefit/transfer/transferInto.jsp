...
<body>
	...
	<ta:form id="form1" cssStyle="overflow:auto;" fit="true">
		...
		<ta:panel id="fld_emp" cols="3" key="基本信息">
			...
			<ta:box cols="2" >
				<ta:text id="aab001" key="失业代管单位代码" labelWidth="130" required="true" minLength="1" bpopTipMsg="可输入失业代管单位代码、单位名称查询" onChange="callSYDGDwRPC();" columnWidth="0.9"/>
				<ta:buttonLayout align="left" columnWidth="0.1">
					<ta:button id="showSYDGDwRPCBtn" key="?" onClick="showSYDGDwRPC()"/>
				</ta:buttonLayout>
			</ta:box>
		...
		<ta:panel id="zrjjxx" key="转入基金信息" bodyStyle="margin:0px">
			<ta:datagrid id="ad12List" selectType="radio" height="90px">
				<ta:datagridItem showDetailed="true" id="yad010" key="登帐id" dataAlign="center" align="center" width="85px" hiddenColumn="true" />
				<ta:datagridItem showDetailed="true" id="aac003" key="姓名" dataAlign="center" align="center" width="85px" />
				<ta:datagridItem showDetailed="true" id="aac002" key="身份证号" dataAlign="center" align="center" width="150px" />
				<ta:datagridItem showDetailed="true" id="aae140" key="险种类型" dataAlign="center" align="center" width="185px" collection="AAE140" />
				<ta:datagridItem showDetailed="true" id="yad003" key="处理金额" dataAlign="right" align="right" width="100px" formatter="convertState" dataType="number" />
				<ta:datagridItem showDetailed="true" id="aae009" key="户名" dataAlign="center" align="center" width="220px" />
				<ta:datagridItem showDetailed="true" id="aae010" key="银行账号" dataAlign="center" align="center" width="185px" />
				<ta:datagridItem showDetailed="true" id="aae013" key="备注" dataAlign="center" align="center" width="286px" />
			</ta:datagrid>
		</ta:panel>
		...
		<ta:panel id="zyxx" key="转移前信息" bodyStyle="margin:0px" cols="3">
			<ta:selectInput id="sfbz" key="是否区直转入" required="true" labelWidth="130" collection="SFBZ" value="0" onSelect="CheckYie014" />
			<ta:text id="yie014" key="区直转出证明号" labelWidth="130" required="true" display="false" />
			...
			<ta:number id="ajc071" key="参保缴费月数" max="500" required="true" labelWidth="130" />
			<ta:number id="ajc099" key="剩余享受月数" max="50" required="true" labelWidth="130" />
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		initializeSuggestFramework(2,null,"aab001",400,200,2,fn_getAe10,0,false); //查询人员用，fn_queryInfo是回调函数里面写自己的查询(根据取得的aac001来查询)
	});
	...
	function fn_setRyData(){
		Base.submit("aac001,aac002,aac003,aae140","transferIntoAction!queryBaseInfo.do",null,null,null,
				    function(){	var  grid=Base.getObj("ad12List");
		                        var  rows=[];
		                        rows.push(0);
		                        //设置指定行被选中
	...
	// 单位RPC
	function callSYDGDwRPC(){
		sfwQueryUtil_newRPC(2,"getAb01a1String",{"dto['inputString']":Base.getValue("aab001")});
	}
	
	// 打开单位信息查询页面
	function showSYDGDwRPC(){
        Base.openWindow("newRPCWindow","RPC查询窗口",myPath()+"/newRPCWindows/newRPCWindowsAction!showRPCWindow09.do",null,700,300,null,fn_setSYDGDwData,true);
	}
	
	// 查询单位信息回调函数
	function fn_setSYDGDwData(){
	...
	function fn_queryInfo(){
	   if (g_Suggest!=""){ 
		   ...
		   Base.submit("aac001,aac002,aac003,aae140","transferIntoAction!queryBaseInfo.do",null,null,null,function(){	var  grid=Base.getObj("ad12List");
		   var  rows=[];
		   rows.push(0);
		   //设置指定行被选中
		   grid.setSelectedRows(rows);});   
	...
	function submitData(){
		...
		Base.submit("form1","transferIntoAction!toSave.do",prm,null,null,null);
	}
