...
<body>
	...
	<ta:form id="form1" fit="true" cssStyle="overflow:auto;">
		...
		<ta:panel id="paymentInfo" key="" hasBorder="false" >
			<ta:panel id="ffxxsyqk" key="其他信息录入" cols="3">
				...
				<ta:selectInput id="aaa036" key="待遇类型" labelWidth="130" data="[{'id':'400010','name':'工伤医疗费'},{'id':'400012','name':'辅助器具费'}]"
							required="true" onSelect="fnChange"  collection="AAA036" value="400010" readOnly="true"/>
			</ta:panel>
		</ta:panel>
		<ta:panel id="gsylfbxDetailHisPanel" key="工伤医疗费详细信息(未核定)">
			<ta:datagrid id="gsylfbxDetailHisGrid" onRowDBClick="clickFn" haveSn="true" height="100px" selectType="radio">
				...
				<ta:datagridItem id="yic380" key="统筹负担金额" width="115" />
				<ta:datagridItem id="yic381" key="单位负担金额" width="115" />
		...
		<ta:panel id="gsfzqjfbxDetailHisPanel" key="工伤辅助器具详细信息(未核定)">
			<ta:datagrid id="gsfzqjfbxDetailHisGrid" forceFitColumns="true" haveSn="true" height="100px">
				<ta:datagridItem id="aaz127" key="工伤认定ID" hiddenColumn="false" />
				...
				<ta:datagridItem id="aaz105" key="辅助器具项目ID" hiddenColumn="false" />
				<ta:datagridItem id="ala020" key="项目名称" />
				<ta:datagridItem id="akc225" key="辅助器具单价" />
				<ta:datagridItem id="akc226" key="配置数量" />
				<ta:datagridItem id="alc010" key="配置时间" />
...
<script type="text/javascript">
	...
	function showRyRPC() {
		Base.openWindow("newRPCWindow", "RPC查询窗口", myPath() + "/newRPCWindows/newRPCWindowsAction!showRPCWindow01.do", null, 1000, 450, null, function(){
			Base.submit("","medicalCalculateAction!queryPersonInfo.do",{"dto['aac001']":Base.getValue("aac001"),"dto['aae140']":Base.getValue("aae140")});
		}, true);
	}
	...
	function fnGsxxSelectChange(rowsData, n) {
		...
		Base.setValue("aab001", rowsData.aab001);
	 	...
	 	Base.setValue("alc031",rowsData.alc031);
		Base.submit("", "medicalCalculateAction!queryUnitInfo.do", {
	...
	function queryHisInfo() {
		...
		Base.submit("procInc,jbxxfset","medicalCalculateAction!queryHisInfo.do", {
					"dto['aaz127']" : aaz127,
					"dto['aaa036']" : aaa036,
					"dto['aac001']" : Base.getValue("aac001")
				});
	...
	function submitData() {
		...
		var rowdata = Base.getGridSelectedRows("gsylfbxDetailHisGrid");
	    if (rowdata == '' || rowdata == null){
	        alert("请选择要核定的医疗费！");
	        return false;
	     }	
		var aaz194 = rowdata[0].aaz194;
		Base.submit("form1", "medicalCalculateAction!toSave.do",{"dto['aaz194']":aaz194},null,null,function(){
	...
	function print(){
		...
		$("#report1_printIFrame").attr("src","<%=path%>/reportServlet?action=2&name=report1&reportFileName=l_dyhd_gsylhdspb.raq&"+
